...
if (isset($_POST['accesskey'])) {
    ...
    if ($access_key_received == $access_key) {
        ...
        if (isset($_POST['category']) && trim($_POST['category']) != "") {
            ...
            if (empty($where)) {
                $where = " WHERE p.category_id IN($category_id) AND p.status = 1 AND pv.stock >= 0";
            } else {
                $where .= " AND p.category_id IN($category_id) AND p.status = 1 AND pv.stock >= 0";
            }
        }
        ...
        if (isset($_POST['sub-category']) && trim($_POST['sub-category']) != "") {
            ...
            $category_id = implode(',', $category_ids);
            if (empty($where)) {
                $where = " WHERE p.subcategory_id IN($category_id) AND p.status = 1 AND pv.stock >= 0";
            } else {
                $where .= " AND p.subcategory_id IN($category_id) AND p.status = 1 AND pv.stock >= 0";
            }
        }
        ...
        if (isset($_POST['out_of_stock']) && isset($_POST['out_of_stock']) && $_POST['out_of_stock'] == 1) {
            if (empty($where)) {
        ...
        $sql = "SELECT p.*, c.name as category_name,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price, (select MIN(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as min_price, (select MAX(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as max_price FROM products p LEFT JOIN category c on c.id=p.category_id $where ";
        ...
        if (isset($_POST['discount_filter']) && isset($_POST['discount_filter']) && intval($_POST['discount_filter'])) {
            $sql = "SELECT p.*,ceil(((price-discounted_price)/price)*100) as cal_discount_percentage,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price, (select MIN(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as min_price, (select MAX(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as max_price FROM products p JOIN product_variant pv on p.id=pv.product_id $where ";

            $sql .= "having cal_discount_percentage > " . $_POST['discount_filter'];
        }
        echo $sql .= "GROUP BY p.id $sort LIMIT $offset, $limit";
        $db->sql($sql);
        $products = $db->getResult();

        $sql = "SELECT p.id, (SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price, (select MIN(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as min_price, (select MAX(if(discounted_price > 0, discounted_price, price)) from product_variant where product_variant.product_id = p.id) as max_price FROM products p $where ";
        if (isset($_POST['min_price']) && isset($_POST['max_price']) && intval($_POST['max_price'])) {
            $sql .= " Having min_price > " . intval(intval($_POST['min_price']) - 1) . " and max_price < " . intval(intval($_POST['max_price']) + 1);
        }
        if (isset($_POST['discount_filter']) && isset($_POST['discount_filter']) && intval($_POST['discount_filter'])) {
