...
if (isset($_GET['table']) && $_GET['table'] == 'users') {
    ...
    $sql = "SELECT u.*,(SELECT name FROM city c WHERE c.id=ua.city_id) as city_name,ua.landmark as street,(SELECT name FROM area a WHERE a.id=ua.area_id) as area_name FROM `users` u JOIN user_addresses ua on ua.user_id-u.id " . $where . " ORDER BY `" . $sort . "` " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
    $bulkData = array();
    $bulkData['total'] = $total;
...
if (isset($_GET['table']) && $_GET['table'] == 'blogs') {
    ...
    $sql = "SELECT COUNT(b.`id`) as total FROM `blogs` b " . $where;
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row)
        $total = $row['total'];

    $sql = "SELECT b.*,bc.name as category_name FROM `blogs` b join blog_categories bc on bc.id = b.category_id" . $where . " ORDER BY " . $sort . " " . $order . " LIMIT " . $offset . ", " . $limit;
    $db->sql($sql);
    $res = $db->getResult();
...
// data of 'product advertisement' table goes here
...
if (isset($_GET['table']) && $_GET['table'] == 'complaints') {
    ...
    foreach ($res as $row) {
        ...
        $operate .= ' <a class="btn btn-xs btn-danger" href="delete-product.php?id=' . $row['id'] . '" title="Delete"><i class="fa fa-trash-o"></i></a>&nbsp;';
