...
/*if (isset($_POST['footer_social_media']) && $_POST['footer_social_media'] == 1) {
    if ($permissions['settings']['update'] == 0) {
        echo '<label class="alert alert-danger">You have no permission to update settings</label>';
        return false;
    }
  ...
  // }else{
        //$settings_value = json_encode($fn->xss_clean_array($_POST));
        //$sql = "UPDATE settings SET value='" . $settings_value . "' WHERE variable='footer_social_media'";
        //if ($db->sql($sql)) {
        //    echo "<p class='alert alert-success'>Saved Successfully!</p>";
        //} else {
            //echo "<p class='alert alert-danger'>Something went wrong please try again!</p>";
    ...
    if ($_FILES["upload_file"]["size"] > 0  && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $emapData[11] = trim($db->escapeString($fn->xss_clean($emapData[11]))); // other_image
                $slug = $function->slugify($emapData[0]);
                $sql = "INSERT INTO products (`name`,`slug`,`category_id`,`subcategory_id`,`indicator`,`manufacturer`,`made_in`,`return_status`,`cancelable_status`,`till_status`,`description`,`image`,`other_images`) VALUES ('" . $emapData[0] . "','" . $slug . "','" . $emapData[1] . "','" . $emapData[2] . "','" . $emapData[3] . "','" . $emapData[4] . "','" . $emapData[5] . "','" . $emapData[6] . "','" . $emapData[7] . "','" . $emapData[8] . "','" . $emapData[9] . "','" . $emapData[10] . "','" . $emapData[11] . "')";
                $db->sql($sql);
            }
    ...
    if ($_FILES["upload_file"]["size"] > 0  && $error == false) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count != 0) {
                ...
                $emapData[12] = $db->escapeString($fn->xss_clean($emapData[12])); // other_images
                // print_r($emapData);
                // return false;
                if (empty($emapData[0])) {
        ...
        while (($emapData = fgetcsv($file, 10000, ",")) !== FALSE) {
            if ($count1 != 0) {
                ...
                $emapData[12] = !empty($emapData[12]) ? trim($db->escapeString($fn->xss_clean($emapData[12]))) : $result[0]['other_images']; // other_images

                $slug = !empty($emapData[1]) ? $function->slugify($emapData[1]) : $result[0]['slug'];
                $sql = "UPDATE products SET `name`='" . $emapData[1] . "',`slug`='" . $slug . "',`category_id`='" . $emapData[2] . "',`subcategory_id`='" . $emapData[3] . "',`indicator`='" . $emapData[4] . "',`manufacturer`='" . $emapData[5] . "',`made_in`='" . $emapData[6] . "',`return_status`='" . $emapData[7] . "',`cancelable_status`='" . $emapData[8] . "',`till_status`='" . $emapData[9] . "',`description`='" . $emapData[10] . "',`image`='" . $emapData[11] . "',`other_images` = '" . $emapData[12] . "' WHERE id=" . $emapData[0];
                $db->sql($sql);
            }
