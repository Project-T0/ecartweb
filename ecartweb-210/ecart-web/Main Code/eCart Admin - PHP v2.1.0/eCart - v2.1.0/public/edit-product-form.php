...
if (isset($_POST['btnEdit'])) {
    ...
    if ($permissions['products']['update'] == 1) {
        ...
        $image_error = $db->escapeString($fn->xss_clean($_FILES['size_chart']['error']));
        $image_type = $db->escapeString($fn->xss_clean($_FILES['size_chart']['type']));
        ...
        $extension = end(explode(".", $_FILES["size_chart"]["name"]));
        ...
        if (!empty($name) && !empty($category_id) &&  !empty($serve_for) && !empty($description) && empty($error['cancelable']) && empty($error)) {
            ...
            if (!empty($size_chart)) {
                ...
                $size_chart = $function->get_random_string($string, 4) . "-" . date("Y-m-d") . "." . $extension;
            ...
            if (
                ...
                $insert_packate_measurement = $db->escapeString($fn->xss_clean($_POST['insert_packate_measurement_unit_id']));
            ...
            if (
                ...
                $insert_loose_measurement = $db->escapeString($fn->xss_clean($_POST['insert_loose_measurement_unit_id']));
                                    ...
                                    <div class="row packate_div">
                                        ...
                                        <div class="col-md-2">
                                            <div class="form-group packate_div">
                                                <label for="exampleInputEmail1">Measurement1</label> <i class="text-danger asterik">*</i> <input type="number" step="any" min="0" class="form-control measurement" name="packate_measurement[]" value='<?= $row['measurement']; ?>' required />
                                            </div>
                                        </div>
                                        <div class="col-md-1">
                                            <div class="form-group packate_div">
                            ...
                            <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <img src="<?php echo $data['size_chart']; ?>" width="210" height="160" />
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label for="exampleInputEmail1">shipping_delivery :<?php echo isset($error['description']) ? $error['description'] : ''; ?>
                                            <textarea name="shipping_delivery" id="shipping_delivery" class="form-control" rows="2"><?php echo $data['shipping_delivery']; ?></textarea>
                                            <script type="text/javascript" src="css/js/ckeditor/ckeditor.js"></script>
                                            <script type="text/javascript">
...
<script>
    $(document).on('change', '.loose_units', function() {
        id = $('.loose_units').val();
        $.ajax({
            type: 'POST',
            url: "public/db-operation.php",
            data: 'accesskey=90336&get_units_of_products=1&unit_id=' + id,
            success: function(result) {
                if (result == 1) {
